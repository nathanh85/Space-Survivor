...
export default class FlightScene extends Phaser.Scene {
  ...
  update(time, delta) {
    ...
    // Out-of-fuel mechanic (speed reduction handled by speedMult in twin-stick input block)
    ...
    if (this.currentSystem) {
      ...
      if (distToStar < star.radius * 2.5 && this.player.body) {
        ...
        // Dot product: movement velocity vs away-from-star direction
  ...
  updateCombat(time, delta) {
    ...
    // Fire weapon — left click / gamepad L1 (ptr declared in twin-stick block above)
